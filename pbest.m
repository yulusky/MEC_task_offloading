...
for i=1:M
    ...
    power(10,ak_db/10))/(1-p0)-1))*1e3)-power(10,PMC_max_dbm/10)*1e-3;
    PMC_max_PKD_max(i)=fun(PKD_max_dbm)+power(10,PMC_max_dbm/10)*1e-3;
    %dbm单位
    PKD_max_PMC_max(i)=bisect1(fun,0,30,0.0005);
    %dbm表示结果
    PMC_max_PKD_max_dbm(i)=10*log10(PMC_max_PKD_max(i)*1e3);
    PMC_best_dbm(i)=min(PMC_max_PKD_max_dbm(i),PMC_max_dbm);
